...
void main() {
    ...
    // change the normals only if the color is more blue than green
    float isBlue = step(color.g, 1.5*color.b);
    ...
    vec2 shiftedUV1 = uv + isBlue * (0.1 * noise + vec2(0.1 * sin(time), 0.1 * cos(time)));
    vec2 shiftedUV2 = uv + isBlue * (0.1 * noise + vec2(0.1 * cos(time), 0.1 * sin(time)));
    ...
    vec3 modifiedNormal = normal + vec3(isBlue) * (vec3(5.0)*interpolatedTex);
